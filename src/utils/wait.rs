...
pub fn wait_for_screen_text_or_timeout(
    kitty: &KittyHarness,
    timeout: Duration,
    predicate: impl Fn(&str) -> bool,
...
pub fn wait_for_screen_text_clean(
    ...
    wait_for_screen_text_clean_or_timeout(kitty, timeout, predicate)
        .unwrap_or_else(|err| (err.last_raw, err.last_clean.unwrap_or_default()))
}
...
pub fn wait_for_clean_contains(kitty: &KittyHarness, timeout: Duration, needle: &str) -> String {
    let (_raw, clean) =
        wait_for_screen_text_clean(kitty, timeout, |_raw, clean| clean.contains(needle));
    clean
}
